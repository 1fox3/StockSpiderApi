 ...
 */
@Component
public class SinaPriceDealNumRatioApi extends SinaBaseApi {
    /**
     * 日志
     */
    ...
    private List<SinaPriceDealNumRatioPo> handleResponse(String response) {
        ...
        while (matcher.find()) {
            ...
            //匹配价格
            if (1 == i) {
                sinaPriceDealNumRatioPo.setPrice(BigDecimalUtil.initPrice(e));
            }
            //匹配成交量
            if (2 == i) {
            ...
            if (3 == i) {
                //去掉占比的百分号
                e = e.replace("%", "");
                sinaPriceDealNumRatioPo.setRatio(BigDecimalUtil.initPrice(e));
